...
<HARD-GATE>
Do NOT create any deliverable, write any .docx file, invoke any builder agent, or start
...
**Options:**
- **Teacher Lesson Plan (.docx)** - "Required: 6-day detailed plans with bellringer keys, scripts, ESOL strategies, feedback timing"
- **Student Packet (.docx)** - "Required: Day-by-day format following student-packet-design-guide (Arial, 12pt, sentence frames, organizers)"
- **Answer Key & Exemplars (.docx)** - "Complete responses with text evidence, STOP justifications, and CR model response at chosen tier"
- **Feedback Forms (.docx)** - "Practice→feedback→revise→submit forms (see feedback-system skill)"
- **Exit Tickets (.docx)** - "Daily formative assessments (1-2 questions per day)"
- **Cover Pages (.docx)** - "Unit overview with benchmarks, routines, materials list"
...
✓ Student Packet (.docx) — follows student-packet-design-guide
...
4. **Build in this order**:
   - Phase 1: Gather & Analyze (read materials, analyze benchmark)
   - Phase 2: Plan Structure (18 vocab words, 6-day outline, ESOL strategies, CR framework + tier from Tab 3)
   - Phase 3: Build Lesson Plan (.docx) — Include selected activities per day, CR scaffolding progression, feedback timing
   - Phase 4: Build Student Packet (.docx) — Day-by-day format following `student-packet-design-guide` specs
   - Phase 5: Build Answer Key (.docx) — Complete exemplars including CR model response at chosen tier
...
6. **Deliver files**:
   - Generate all files with proper naming: `[PassageTitle]_[Type]_[MM-DD].docx`
...
[Present Tab 5: Deliverables — which .docx files to build]
...
| **Deliverables** (Tab 5) | `unit-builder-protocol`, `teaching-templates`, `file-management` | Which .docx files to generate, file naming |
...
INVOKE: lesson-plan-coordinator AGENT
  ...
  RECEIVE: Teacher Lesson Plan (.docx)
...
IF esol_adapter_needed:
  INVOKE: esol-adapter AGENT
    PASS: Student Packet (.docx), class_type, ESOL proficiency levels
    RECEIVE: ESOL-adapted Student Packet (.docx)
...
INVOKE: unit-reviser AGENT (sync check mode)
  ...
  CHECK:
    ...
    - File naming follows [PassageTitle]_[Type]_[MM-DD].docx
...
### Phase 6: Deliver
```
OUTPUT: All .docx files with proper naming
